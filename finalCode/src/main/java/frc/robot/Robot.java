...
//Global Variables
    ...
    public void robotPeriodic() {
        ...
        int selSenPosTurret = turretMotor.getSelectedSensorPosition(0);
        int currentPosTurret = selSenPosTurret;

        // Turret Adjustment
        final double headingError = -Robot.x;
        System.out.println(headingError);
        if (Robot.x > 1 && currentPosTurret <= posLimTurret){
             steeringAdjust = Kp*headingError - min_command;
        }else if(Robot.x < 1 && currentPosTurret >= negLimTurret){
             steeringAdjust = Kp*headingError + min_command;
        }else{
            steeringAdjust = 0;
        }
